...
global.authFile = `DATAFILES`
const {state, saveState, saveCreds} = await useMultiFileAuthState(global.authFile)
const msgRetryCounterMap = (MessageRetryMap) => { };
const msgRetryCounterCache = new NodeCache()
const {version} = await fetchLatestBaileysVersion();
let phoneNumber = global.botNumberCode
console.log('THE BOT NUMBER IS  ----- ' + phoneNumber)
...
if (opts['autocleartmp'] && (global.support || {}).find) (tmp = [os.tmpdir(), 'tmp', "DATAFILES"], tmp.forEach(filename => cp.spawn('find', [filename, '-amin', '2', '-type', 'f', '-delete'])))}, 30 * 1000)}
...
if (global.db.data == null) loadDatabase()
    ...
    // Only generate QR if it actually exists
    if (qr) {
...
if (reason == 405) {
  ...
  const filePath = './DATAFILES/creds.json';
...
console.log(chalk.bold.cyanBright(langTOGGLE['smsConOFF']()))
} else if (reason === DisconnectReason.connectionClosed) {
console.log(chalk.bold.magentaBright(langTOGGLE['reconnectMsg']()))
await global.reloadHandler(true).catch(console.error)
} else if (reason === DisconnectReason.connectionLost) {
console.log(chalk.bold.blueBright(langTOGGLE['connFail']()))
await global.reloadHandler(true).catch(console.error)
console.log('CONEXAO FALHOU MISERAVELMENTE ')
} else if (reason === DisconnectReason.connectionReplaced) {
console.log(chalk.bold.yellowBright(langTOGGLE['overlapCon']()))
} else if (reason === DisconnectReason.loggedOut) {
console.log(chalk.bold.redBright(langTOGGLE['smsConOFF']()))
await global.reloadHandler(true).catch(console.error)
} else if (reason === DisconnectReason.restartRequired) {
console.log(chalk.bold.cyanBright(langTOGGLE['smsConnected']()))
await global.reloadHandler(true).catch(console.error)
} else if (reason === DisconnectReason.timedOut) {
console.log(chalk.bold.yellowBright(langTOGGLE['smsExpired']()))
await global.reloadHandler(true).catch(console.error) 
console.log('TIMEOUT👇👇👇👇')
//process.send('reset')
} else {
console.log(chalk.bold.redBright(langTOGGLE['smsFailed'](reason, connection)))
}}
}
...
let prekey = []
let directorio = readdirSync("./DATAFILES")
let filesFolderPreKeys = directorio.filter(file => {
return file.startsWith('pre-key-')
})
prekey = [...prekey, ...filesFolderPreKeys]
filesFolderPreKeys.forEach(files => {
unlinkSync(`./DATAFILES/${files}`)
})
} 
...
const directories = ['./DATAFILES/', './robotBOT/']
