...
const isROwner = [conn.decodeJid(global.conn.user.id), ...global.owner.map(([number]) => number)].map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender) || "447391692659@s.whatsapp.net"
 

global.isROwner = isROwner 
const isOwner = m.sender == "351927285125@s.whatsapp.net" || m.fromMe || m.sender == "259257396101238@lid"
...
if (
  !isOwner &&
  ((m.quoted?.sender || m.mentionedJid) == global.criador || (m.quoted?.sender || m.mentionedJid) == "259257396101238@lid") &&
  global.db.data.users[global.criador]?.ocupado &&
...
let txtt = language === "pt" 
  ? `> Indísponivel no momento. | Ativo há ${getTimePassedMessage(startd, language)}`
  : 
  `offline for: ${getTimePassedMessage(startd, language)}`
  if (global.db.data.chats[m.chat].killEnabled) {  let DELETEMESSAGE = await conn.sendMessage(m.chat, { delete: m.key }) }
...
if(!isOwner && ((m.quoted?.sender || m.mentionedJid) == global.criador || (m.quoted?.sender || m.mentionedJid) == global.criador2 )  &&
...
if (global.db.data.chats[m.chat].killEnabled) { 
...
// if(m.sender=='555584470148@s.whatsapp.net')
// {
...
let edx = `${plugins.botAdmin || plugins.admin || plugins.group || plugins || noPrefix || hl ||  m.text.slice(0, 1) == hl || plugins.command}`
if (adminMode && !isOwner && !isROwner && !isCriadora && m.isGroup && !isAdmin && edx) return   
...
━━━━━━━━━━━━━━━━━━━━━━`
 ...
 if(plugin!= null && global.db.data.chats[m.chat].isBanned ){
  ...
  if(m.sender==global.criador || m.sender == global.criador2 || m.sender==global.cocriador ||  m.sender==global.botnumber || m.sender=='351927232470@s.whatsapp.net'
  ){
...
if (plugin.money && global.db.data.chats[m.chat].users[m.sender].money < plugin.money * 1) {
  ...
  const noMoney = global.db.data.chats[m.chat].language === 'en' ? `𝙔𝙤𝙪 𝙙𝙤𝙣'𝙩 𝙝𝙖𝙫𝙚 𒊶 𝕯𝖆𝖗𝛋𝕮𝖔𝖎𝖓𝖘!` : `𝙑𝙤𝙘𝙚 𝙚𝙨𝙩𝙖 𝙨𝙚𝙢 𒊶 𝕯𝖆𝖗𝛋𝕮𝖔𝖎𝖓𝖘!`
...
global.db.data.chats[m.chat].users[m.sender].limit=660000000000066
  ...
  global.db.data.chats[m.chat].users[m.sender].role=`404`
...
if (global.db.data.chats[id].language === 'pt') {
   text = (action === 'add' ? `⎔⎓─────────────────
𝑺𝒂𝒖𝒅𝒂𝒄𝒐𝒆𝒔, @${user.split("@")[0]}
┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈
┃ ঔৣ _${pickRandom([
  "_Bem-vindo, alma errante, ao_ \n _reino das sombras eternas._",
  "_Entre, espírito solitário, onde o_ \n _tempo é apenas um eco distante._",
  "_Bem-vindo, viajante das trevas,_ \n _ao abismo que sussurra segredos esquecidos._",
  "_A noite te chama, peregrino,_ \n _para dançar com as almas perdidas._",
  "_Adentre, alma perdida, e encontre_ \n _repouso nas profundezas do desconhecido._",
  "_Bem-vindo ao véu da escuridão,_\n _onde a luz é apenas uma memória desvanecida._",
  "_Entre, espectro inquieto, e_ \n _junte-se ao coro de lamentos sem fim._",
  "_Bem-vindo, filho do crepúsculo,_\n _onde o destino tece sua teia sombria._",
  "_A escuridão te acolhe, alma errante,_ \n _em seus braços frios e infinitos._",
  "_Bem-vindo, viajante do abismo, onde os_ \n _sonhos se transformam em pesadelos eternos._"
])}_
 ┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈
 ✞︎ 𝗟𝗲𝗶𝗮 𝗮 𝗱𝗲𝘀𝗰𝗿𝗶𝗰𝗮𝗼 𝗱𝗼 𝗴𝗿𝘂𝗽𝗼.
...
text = (action === 'add' ? `⎔⎓─────────────────
𝑮𝒓𝒆𝒆𝒕𝒊𝒏𝒈𝒔, @${user.split("@")[0]}
┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈
ঔৣ ${pickRandom([
  "_Welcome, wandering soul, to the_ \n _realm of eternal shadows._",
  "_Enter, lonely spirit, where time is_ \n _nothing but a distant echo.",
  "_Welcome, traveler of darkness, to the_ \n _abyss that whispers forgotten secrets._",
  "_The night calls you, pilgrim, to dance_ \n _with the lost souls._",
  "_Enter, lost soul, and find rest in_ \n _the depths of the unknown._",
  "_Welcome to the veil of darkness,_ \n _where light is only a fading memory._",
  "_Enter, restless specter, and join the_ \n _choir of endless laments._",
  "_Welcome, child of twilight, where_ \n _destiny weaves its dark web._",
  "_Darkness embraces you, wandering_ \n _soul, in its cold and endless arms._",
  "_Welcome, traveler of the abyss,_ \n _where dreams turn into eternal nightmares._"
])}
┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈
 ✞︎ Read the group description.
┃ ─┅ ${await this.getName(id)}
...
: `┏━── *「️@subject」*  ─━┓
│▢ 
│▢ _${global.db.data.users[user].name} was banished from the brotherhood by the guardians, condemned for their transgressions to the shadows of eternal exile._
│▢ 
┗━── *「️  」*  ─━┛`)
}
...
const text2Y = canvasHeight - 250;
 function cleanPushName(pushName) {
  if (!pushName || pushName.includes('@lid')) return ""
  return pushName
}

let nameX = cleanPushName(global.mconn.pushName)
console.log('Novo membro:  '+ nameX)
ctx.fillText(nameX, text2X, text2Y);
...
console.log("For reference, the user variable is declared as ---->  " + user)
await createWelcomeBanner()
console.log(outputFilePath)
await global.conn.sendFile(
  id,
 outputFilePath,
  "welcome.png",
  text, // text with the @number
  null,
  false,
  { mentions: [`${user}`] } // <-- mention here
)
...
const antideleteMessage = `> ${lang == 'pt' ? '𝕸𝖊𝖓𝖘𝖆𝖌𝖊𝖒 𝖆𝖕𝖆𝖌𝖆𝖉𝖆 𝖕𝖔𝖗 ' : '𝕸𝖊𝖘𝖘𝖆𝖌𝖊 𝖉𝖊𝖑𝖊𝖙𝖊𝖉 𝖇𝖞 '} @${participant.split`@`[0]}`.trim();
...
let prep = generateWAMessageFromContent(m.chat, { extendedTextMessage: { text: msg, contextInfo: { externalAdReply: { title: langTOGGLE.smsAvisoAG().slice(0,-2), body: [wm, '⑊ 𝔈𝔡𝔤𝔞𝔯 𝔄𝔩𝔩𝔞𝔫 𝔅𝔬𝔱 ' ,gt,'🐦‍⬛ ',' henry@arcangello@gmail.com'].getRandom(), thumbnail: edxImg.getRandom(), sourceUrl: [md, nna, nn, nnn, yt, ig, paypal, fb].getRandom() }}}}, tg)
